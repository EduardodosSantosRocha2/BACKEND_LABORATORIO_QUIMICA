...
class Experimento {
    ...
    private $table_name2 = 'Realiza_experimento';
    ...
    public function create($fk_Aula_ID_aula,$fk_Experimento_Numero_Experimento, $Observacao) {
        $query = "INSERT INTO " . $this->table_name2 . " (fk_Aula_ID_aula, fk_Experimento_Numero_Experimento, Observacao)
        VALUES (:fk_Aula_ID_aula, :fk_Experimento_Numero_Experimento, :Observacao)";
        $stmt = $this->conn->prepare($query);
        $stmt->bindParam(":fk_Aula_ID_aula", $fk_Aula_ID_aula);
...
function handleRequest($conn) {
    if ($_SERVER['REQUEST_METHOD'] === 'GET') {
        ...
        $Experimento = new Experimento($conn);
        if ($Experimento->create($id,$numero, $obs)) {
            echo json_encode(array('message' => 'Experimento criado com sucesso.', 'status'=> true));
        } else {
            echo json_encode(array('message' => 'Não foi possível criar o experimento.','status'=> false));
        }
