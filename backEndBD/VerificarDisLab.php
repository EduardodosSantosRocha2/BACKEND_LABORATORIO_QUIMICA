...
public function verificarDisponibilidadeLaboratorio($datar, $horario) {
        $query = "SELECT VerificarDisponibilidadeLaboratorio(:datar, :horario)";
        $stmt = $this->conn->prepare($query);
        $stmt->bindParam(":datar", $datar);
        $stmt->bindParam(":horario", $horario);
        $stmt->execute();
        
        $result = $stmt->fetch(PDO::FETCH_ASSOC);
        
        $valor = $result['verificardisponibilidadelaboratorio'];
...
function handleRequest($conn) {
    if ($_SERVER['REQUEST_METHOD'] === 'GET') {
        ...
        $consulta = $reserva->verificarDisponibilidadeLaboratorio($datar,$horario);

        if ($consulta === 1) {
            echo json_encode(array('message' => 'Horario indisponivel!','valor'=>$consulta));
        } else {
            echo json_encode(array('message' => 'Horario disponivel!.','valor'=>$consulta));
        }
    } 
}
